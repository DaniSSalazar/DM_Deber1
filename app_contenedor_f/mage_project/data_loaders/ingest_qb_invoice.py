...
def _fetch_qb_by_date(realm_id, access_token, query_base, base_url, minor_version, page_size=100, max_retries=5):
    headers = {
    ...
    while True:
        ...
        while True:
            try:
                resp = requests.get(
                    f"{base_url.rstrip('/')}/v3/company/{realm_id}/query",
                    headers=headers,
                    params=params,
                    timeout=240
                )
                ...
                if resp.status_code == 429:  # rate limit
                    wait = min(2 ** retries + random.uniform(0, 1), 60)  # exponential backoff con jitter
                    print(f"Rate limit alcanzado. Reintentando en {wait:.1f}s...")
                    time.sleep(wait)
                    retries += 1
                    if retries > max_retries:
                        raise Exception("Max reintentos alcanzados por rate limit")
                    continue

                resp.raise_for_status()
                break  # salió del loop si fue exitoso

            except requests.exceptions.RequestException as e:
                wait = min(2 ** retries + random.uniform(0, 1), 60)
                print(f"Error en request ({e}), reintento {retries+1} en {wait:.1f}s")
                time.sleep(wait)
                retries += 1
                if retries > max_retries:
                    raise
        ...
        data = resp.json()
        rows = data.get("QueryResponse", {}).get("Item", [])
...
def load_data_from_api(*args, **kwargs):
    ...
    print(f"Total invoices descargadas: {len(all_data)}")
    return all_data
...
def test_output(output, *args) -> None:
    ...
    print(f"Se descargaron {len(output)} invoices.")
